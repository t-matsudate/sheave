...
impl Decoder<EcmaArray> for ByteBuffer {
    ...
    /// ```
    ///
    /// Note the length field will not be so cared because to decodde is enough to check the object end marker (0x09).
    ...
    fn decode(&mut self) -> IOResult<EcmaArray> {
        ...
        if properties.len() != length as usize {
            // TODO: Replaces something logger later.
            println!("Properties length doesn't match previous field: previous field: {length}, actual length: {}", properties.len());
        }
